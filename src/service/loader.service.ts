...
export class LoaderService {
  constructor(private readonly carbonConsumerService: ConsumerService) {}

  async startBulkLoad() {
    const rl = createInterface({
      input: createReadStream(
        join(__dirname, '..', 'data', 'initial-list.txt'),
      ),
    });
  ...
  private throttleCarbinFetch(url: string, index: number) {
    return setTimeout(
      () =>
        this.handleResult(this.carbonConsumerService.getCarbonStatistic(url)),
      3000 * index,
    );
  }
  ...
  private async handleResult(carbonResult: Promise<ConsumerDto>) {
    try {
      console.log(await carbonResult);
    } catch (error) {
